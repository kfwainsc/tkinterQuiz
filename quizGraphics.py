...
#  the QuizGui class for running the application
#########################################################
from tkinter import *
...
class QuizGui(Quiz):
    ...
    def __init__(self, root, title, questList):
    ...
    def makeChoiceBtns(self):
        self._choiceBtns = self._qList[self._cur].choiceBtns(root, self._selected)
        #starting pos
        y_pos = 300 
    ...
    def showTitle(self, inTitle):
        title = Label(root, text=inTitle, wraplength=1000, justify=CENTER, anchor='w', height=3, width=100,
    ...
    def showQuestion(self):
        ...
        self._questLabel = Label(root, text=self._qList[self._cur].get('Q'),
    ...
    def next_check_quit_btns(self):
        # CHECK BUTTON
        checkBtn = Button(root, text="Check", command=self.check,
                   width=10,bg="black",fg="white",font=("ariel",16,"bold"))
        checkBtn.place(x=250,y=610)
        # NEXT BUTTON
        nextBtn = Button(root, text="Next", command=self.next,
                  width=10,bg="black",fg="white",font=("ariel",16,"bold"))
    ...
    def check(self):
        if self.checkAnswer(self._selected.get()):
            self._message = Label(root, text="Good Work!", anchor='w', 
                            width=55, fg="green", font=("ariel",14,"bold"))
        else:
            self._message = Label(root, text="Correct Answer:", width=55,
                            anchor='w', fg="red", font=("ariel",14,"bold"))
        self._message.place(x=700, y=300)       
        self._resourceLabel = self._qList[self._cur].resourceLabel(root)
...
root.title(quizTitle)
#c = Canvas(gui, bg="gray", height=300, width=300)
#c.pack()
l = Label(root, text="Good Work!", anchor='w', 
          width=55, fg="green", font=("ariel",14,"bold"))
quiz = QuizGui(root, quizTitle, inQuestions)
root.mainloop()
